...
class CompanyController extends Controller
{
    ...
    public function pending_requests(Request $request)
    {
        ...
        $zone = is_numeric($zone_id)?Zone::findOrFail($zone_id):null;
        return view('admin-views.vendor.company_pending_requests', compact('stores','zone','type', 'search_by'));
    }
    ...
    public function updateStoreSettings(Store $store, Request $request)
    {
        ...
        // if($request->parcel_commission_status)
        // {
        //     $store->parcel_commission = $request->parcel_commission;
        // }
        // else{
        //     $store->parcel_commission = null;
        // }
        ...
        try{
            if($request->status==1){
                ...
                if ( config('mail.status') && $mail_status == '1') {
                    Mail::to($store?->vendor?->email)->send(new \App\Mail\VendorSelfRegistration('approved', $store->vendor->f_name.' '.$store->vendor->l_name,'company'));
                }
            }else{
                $mail_status = Helpers::get_mail_status('deny_mail_status_store');
                if ( config('mail.status') && $mail_status == '1') {
                    Mail::to($store?->vendor?->email)->send(new \App\Mail\VendorSelfRegistration('denied', $store->vendor->f_name.' '.$store->vendor->l_name,'company'));
                }
            }
        }catch(\Exception $ex){
            info($ex->getMessage());
    ...
    public function download_document($fileName){
        ...
        if (Storage::disk('public')->exists($path . $fileName)) {
            return Response::download(storage_path('app/public/store/' . $fileName));
        }
    }
    ...
    {
        return view('admin-views.vendor.company_index');
    }
