...
class StoreLogic
{
    ...
    {
        $paginator = Store::IsStore()->withOpen($longitude??0,$latitude??0)
            ->withCount(['items','campaigns'])
            ->with(['discount'=>function($q){
                return $q->validate();
            }])
    ...
    {
        $paginator = Store::IsStore()->withOpen($longitude??0,$latitude??0)->whereNotNull('rating')
            ->withCount(['items','campaigns'])
            ->with(['discount'=>function($q){
                return $q->validate();
            }])
    ...
    {
        return Store::IsStore()->withOpen($longitude??0,$latitude??0)->with(['discount'=>function($q){
            return $q->validate();
