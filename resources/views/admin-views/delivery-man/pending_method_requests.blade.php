            ...
            <div class="table-responsive datatable-custom">
                <table id="columnSearchDatatable"
                    ...
                    <tr>
                        ...
                        <th class="border-0">{{translate('messages.Delivery Man Name')}}</th>
...
{{--                        <th class="border-0">{{translate('messages.Owner Information')}}</th>--}}
                    ...
                    @foreach($disbursementWithdrawlMethods as $key=>$disbursementWithdrawlMethod)
                        @if(!empty($disbursementWithdrawlMethod->deliveryMan))
                        <tr>
                            <td>{{$key+$disbursementWithdrawlMethods->firstItem()}}</td>
                            <td>
                                <div>

                                    <a href="{{route('admin.users.delivery-man.preview', $disbursementWithdrawlMethod->delivery_man_id)}}" class="table-rest-info" alt="view store">
                                        @if(!empty($disbursementWithdrawlMethod->deliveryMan))
                                            <img class="img--60 circle onerror-image" data-onerror-image="{{asset('public/assets/admin/img/160x160/img1.jpg')}}"
                                            src="{{ \App\CentralLogics\Helpers::get_image_helper(
                                                $disbursementWithdrawlMethod->deliveryMan,'image',
                                                asset('storage/app/public/delivery-man').'/'.$disbursementWithdrawlMethod->deliveryMan['image'] ?? '',
                                                asset('public/assets/admin/img/160x160/img1.jpg'),
                                                'store/'
                                            ) }}" >
                                        @endif
                                        <div class="info"><div class="text--title">
                                            {{Str::limit($disbursementWithdrawlMethod->deliveryMan->f_name.' '.$disbursementWithdrawlMethod->deliveryMan->l_name,20,'...')}}
                                            </div>
                                            <div class="font-light">
                                                {{translate('messages.id')}}:{{$disbursementWithdrawlMethod->delivery_man_id ?? 0}}
                                            </div>
                                        </div>
                                    </a>
                                </div>
...
{{--                            </td>--}}
                            ...
                            <td>
                                @if(!empty($disbursementWithdrawlMethod->deliveryMan))
                                    @foreach(json_decode($disbursementWithdrawlMethod->method_fields) as $key => $field)
