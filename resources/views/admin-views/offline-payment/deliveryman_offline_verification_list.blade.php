...
@section('content')
                ...
                <div class="col-md-12">
                    <div class="js-nav-scroller hs-nav-scroller-horizontal">
                        ...
                        <ul class="nav nav-tabs mb-3 border-0 nav--tabs">
                            <li class="nav-item">
                                <a class="nav-link {{ $status ==  'all' ? 'active' : ''}}" href="{{ route('admin.transactions.delivery_man.offline_payment.delivery_man_offline_verification_list', ['all']) }}"   aria-disabled="true">{{translate('messages.All')}}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link  {{ $status ==  'pending' ? 'active' : ''}}" href="{{ route('admin.transactions.delivery_man.offline_payment.delivery_man_offline_verification_list', ['pending']) }}"  aria-disabled="true">{{translate('messages.Pending')}}</a>
                            </li>
                            <li class="nav-item  {{ $status ==  'verified' ? 'active' : ''}}">
                                <a class="nav-link" href="{{ route('admin.transactions.delivery_man.offline_payment.delivery_man_offline_verification_list', ['verified']) }}"  aria-disabled="true">{{translate('messages.verified')}}</a>
                            </li>
                            <li class="nav-item  {{ $status ==  'denied' ? 'active' : ''}}">
                                <a class="nav-link" href="{{ route('admin.transactions.delivery_man.offline_payment.delivery_man_offline_verification_list', ['denied']) }}"  aria-disabled="true">{{translate('messages.Denied')}}</a>
                            </li>
                        </ul>
                        <!-- End Nav -->
                    </div>
            ...
            <div class="table-responsive datatable-custom">
                <table id="datatable"
                    ...
                    <tr>
                        ...
                        <th class="border-0">{{translate('messages.deliveryman_information')}}</th>
                        ...
                        <tr class="status-{{$order['status']}} class-all">
                            ...
                            <td>
                                @if($order->delivery_man)
                                    <a class="text-body text-capitalize" href="{{route('admin.users.delivery-man.preview', ['id'=>$order->delivery_man->id, 'tab'=> 'info'])}}">
                                        <strong>{{ $order->delivery_man?->f_name }} {{ $order->delivery_man?->l_name }}</strong>
                                        <div>{{$order->delivery_man['phone']}}</div>
                                    </a>
                                @else
                                    <label class="badge badge-danger">{{translate('messages.invalid_delivery_man_data')}}</label>
                                @endif
                            </td>
        ...
        <div class="modal fade" id="verifyViewModal-{{ $key }}" tabindex="-1" aria-labelledby="verifyViewModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="d-flex align-items-center flex-column gap-3 text-center">
                 ...
                 <div class="card">
                    <div class="card-body">
                        <h4 class="mb-3">{{ translate('messages.deliveryman_information') }}</h4>
                            <div class="d-flex flex-column gap-2">
                                @if($order->delivery_man)
                                <div class="d-flex align-items-center gap-2">
                                    <span>{{translate('Name')}}</span>:
                                    <span class="text-dark"> <a class="text-body text-capitalize" href="{{route('admin.users.delivery-man.preview', ['id'=>$order->delivery_man->id, 'tab'=> 'info'])}}"> {{$order->delivery_man['f_name']}} {{$order->delivery_man['l_name']}}  </a>  </span>
                                </div>

                                <div class="d-flex align-items-center gap-2">
                                    <span>{{translate('Phone')}}</span>:
                                    <span class="text-dark">{{$order->delivery_man['phone']}}  </span>
                                </div>
                        ...
                        <div class="btn--container justify-content-end mt-3">
                            ...
                            @elseif ($order->status == 'denied')
                                <button type="button" data-url="{{ route('admin.transactions.delivery_man.offline_payment.verifications', [ 'id' => $order['id'], 'verify' => 'switched_to_cod', ]) }}" data-message="{{ translate('messages.Make_the_payment_verified_for_this_order') }}" class="btn btn--success mb-2 route-alert">{{translate('Switched_to_COD')}}</button>
                            @endif

                            <button type="button" data-url="{{ route('admin.transactions.delivery_man.offline_payment.verifications', [ 'id' => $order['id'], 'verify' => 'yes', ]) }}" data-message="{{ translate('messages.Make_the_payment_verified_for_this_store') }}" class="btn btn--primary mb-2 route-alert">{{translate('Yes,_Payment_Received')}}</button>
                        </div>
                        @endif
                    </div>
                </div> 
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.transactions.delivery_man.offline_payment.verifications') }}" method="get">
            ...
            <div class="modal-footer">
                ...
                <button id="transaction_id_mismatched"  type="submit" class="btn btn--danger btn-outline-danger">{{ translate('messages.Confirm_Rejection') }} </button>
                </form>
            </div>
        </div>
    </div>
...
@push('script_2')
    ...
    <script>
       $("#transaction_id_mismatched").click(function() {
