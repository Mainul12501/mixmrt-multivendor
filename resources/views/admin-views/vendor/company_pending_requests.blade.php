                ...
                <div class="col-md-12">
                    <div class="js-nav-scroller hs-nav-scroller-horizontal mt-2">
                        ...
                        <ul class="nav nav-tabs mb-3 border-0 nav--tabs">
                            <li class="nav-item">
                                <a class="nav-link active" href="{{ route('admin.company.pending-requests') }}"   aria-disabled="true">{{translate('messages.pending_courier_companies')}}</a>
                            </li>
                            <li class="nav-item">
                                ...
                                @if(isset($store->vendor->status))
                                    @if($store->vendor->status)
                                        <label class="toggle-switch toggle-switch-sm" for="stocksCheckbox{{$store->id}}">
                                            <input type="checkbox" data-url="{{route('admin.company.status',[$store->id,$store->status?0:1])}}" data-message="{{translate('messages.you_want_to_change_this_company_status')}}" class="toggle-switch-input status_change_alert" id="stocksCheckbox{{$store->id}}" {{$store->status?'checked':''}}>
                            ...
                            <td>
                                <div class="btn--container">
                                    ...
                                    @if (!isset($store->vendor->status))
                                        ...
                                        data-url="{{route('admin.company.application',[$store['id'],0])}}" data-message="{{translate('messages.you_want_to_deny_this_application')}}"
